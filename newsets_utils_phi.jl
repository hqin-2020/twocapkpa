...
function dstar_twocapitals!(d1::Array{Float64,2},
    ...
    for i=1:IJ
        ...
        # println("i: ", i, ", l: ", pii[i], ", r: ", exp(pii[i])/(exp(pii[i])+1), ", z: ", zz[i], ", d1: ", d1_root,  ", d2: ", d2_root, ", c: ", alpha-d1_root*k1a - d2_root*k2a, ", Vr: ", vr, ", V: ", V[i])
...
function value_function_twocapitals(gamma::Float64,
    ...
    #     #  V0[:, j] = range(-1.5, stop=-0.5, length=II);
    #     end
    
    #   V0 = npzread("/project/lhansen/twocapbal/output/Standard_grid_sym_Delta_300_scale_1754_tol_21_15_300_frac_0.0/gamma_8.0_rho_1.00001/model_sym_HS.npz")["V"]
      V0 = preloadV0;
      C0F = preloadcons;
      C0B = preloadcons;
      v = copy(V0);
      ...
      for n=1:maxit
        ...
        #   dstar_twocapitals!(d1_F, d2_F, Vr_F, pii, zz, IJ, fraction, rho, V, model, preloadd1, preloadcons, foc, clowerlim);
        #   dstar_twocapitals!(d1_B, d2_B, Vr_B, pii, zz, IJ, fraction, rho, V, model, preloadd1, preloadcons, foc, clowerlim);
          dstar_twocapitals!(d1_F, d2_F, Vr_F, pii, zz, IJ, fraction, rho, V, model, preloadd1, CF, foc, clowerlim);
          dstar_twocapitals!(d1_B, d2_B, Vr_B, pii, zz, IJ, fraction, rho, V, model, preloadd1, CB, foc, clowerlim);
          cF = copy(CF)
          cB = copy(CB)
          # Drifts
